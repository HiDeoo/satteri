...
function mdFilePathToUrl(mdFilePath: string, pageSourceDir: string, baseUrl: string): URL {
  const pathBelowRoot = relative(pageSourceDir, mdFilePath);
  const pathname = pathBelowRoot.replace(/\\/g, "/").replace(/\.mdx?$/i, "/");
  return new URL(pathname, baseUrl);
}

function tryFindSourceFileForPathname(pathname: string, pageSourceDir: string): string | undefined {
  const possiblePaths = [
...
export function fallbackLang(options: FallbackLangOptions = {}) {
  ...
  return definePlugin({
    meta: {
      name: "fallback-lang",
      description: 'Marks links pointing to untranslated pages with " (EN)" suffix',
    },
    ...
    createOnce(_ctx) {
      return {
        link(node: MdastNode, context: VisitorContext) {
          ...
          const linkSourceFile = tryFindSourceFileForPathname(linkUrl.pathname, pageSourceDir);
